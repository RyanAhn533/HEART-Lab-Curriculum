...
#3. 컴파일, 훈련
# 손실 함수 정의
criterion = nn.MSELoss()  # MSE 손실 함수 (Mean Squared Error)를 정의. 회귀 문제에서 주로 사용
optimizer = optim.Adam(model.parameters(), lr=1e-4)  # Adam 최적화 알고리즘 사용, 학습률은 0.0001로 설정

# 모델 훈련 함수 정의
def train(model, criterion, optimizer, loader):
    epoch_loss = 0  # 에포크 동안의 총 손실을 저장할 변수
    model.train()   # 모델을 훈련 모드로 설정 (드롭아웃, 배치 정규화 등 훈련 중에만 적용되는 레이어를 활성화)
    
    # 데이터 로더에서 배치 단위로 데이터를 가져와 훈련 반복
    for x_batch, y_batch in loader:
        # 데이터를 GPU 또는 CPU의 DEVICE로 이동시키고, y_batch를 (배치 크기, 1) 형식으로 변환
        x_batch, y_batch = x_batch.to(DEVICE), y_batch.to(DEVICE).float().view(-1, 1)
        
        # 옵티마이저의 기울기를 0으로 초기화 (이전 배치의 기울기가 누적되지 않도록 방지)
        optimizer.zero_grad()
        
        # 모델에 입력 x_batch를 전달하여 예측값(hypothesis) 계산
        hypothesis = model(x_batch)
        
        # 손실 함수(MSE)를 사용하여 예측값(hypothesis)과 실제값(y_batch) 간의 오차 계산
        loss = criterion(hypothesis, y_batch)
        
        # 역전파(backpropagation)를 통해 각 가중치에 대한 기울기 계산
        loss.backward()
        
        # 옵티마이저를 통해 가중치 갱신 (계산된 기울기를 반영하여 가중치 업데이트)
        optimizer.step()
        
        # 배치 손실을 에포크 손실에 더해 총 손실을 기록
        epoch_loss += loss.item()
    
    # 에포크의 평균 손실을 반환하여 훈련 성과 평가
    return epoch_loss / len(loader)

# 모델 평가 함수 정의 (훈련과 달리 역전파나 가중치 갱신을 하지 않음)
def evalutate(model, criterion, loader):
    model.eval()  # 모델을 평가 모드로 설정 (훈련 중에만 적용되는 레이어 비활성화)
    epoch_loss = 0  # 에포크 동안의 총 손실을 저장할 변수

    with torch.no_grad():  # 평가 시에는 기울기를 계산하지 않음 (메모리 절약 및 속도 향상)
        # 데이터 로더에서 배치 단위로 데이터를 가져와 평가 반복
        for x_batch, y_batch in loader:
            # 데이터를 DEVICE로 이동
