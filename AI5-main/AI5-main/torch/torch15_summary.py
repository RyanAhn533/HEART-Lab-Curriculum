...
transf = tr.Compose([tr.Resize(56), tr.ToTensor(), tr.Normalize((0.5), (0.5,))]) # minmax 정규화 값에서 0.5뺀 값서 0.5로 나눠줌
#증폭해버렸다
...
class CNN(nn.Module):
    def __init__(self, num_features):
        ...
        self.hidden_layer4 = nn.Linear(16 * 5 * 5, 128)  # 16 * 5 * 5는 Conv2D 이후 나온 feature의 크기
        self.output_layer = nn.Linear(in_features=128, out_features=10)  # out_features는 MNIST 클래스의 수 (10)
...
# model.summary() 텐서플로는 요거임
print(model)

from torchsummary import summary
summary(model, (1,56,56)) # 토치는 채널먼저 batch는 무시
