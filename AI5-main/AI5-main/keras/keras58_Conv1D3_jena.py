...
from tensorflow.keras.layers import Dense, SimpleRNN, LSTM, GRU, Conv1D, MaxPool1D
...
path1 = 'C:\\프로그램\\ai5\\_save\\keras55\\'
print(x_train.shape)
print(x_test.shape)
# (378236, 144, 13)
# (42027, 144, 13)
#2. 모델구성
model = Sequential()
model.add(Conv1D(filters=10, kernel_size=2, input_shape=(144, 13))) # timesteps , features
model.add(Conv1D(filters=10, kernel_size=2, padding='causal'))# timesteps , features
model.add(Flatten())
...
model.fit(x,y, epochs=100, validation_split=0.2, batch_size=512, callbacks=[es,mcp])
...
# print("rmse = ", rmse)
# sample_submission_csv = pd.read_csv(path1 + "sample_submission_jena.csv", index_col=0)
# sample_submission_csv['T (degC)'] = y_pred
# sample_submission_csv.to_csv(path + "jena_안준영.csv")

(144, 1)
rmse =  1.3719206792892549/
걸린시간 :  317.7705056667328